...
still to do:
figure out imports to remove from consumption vec
make version of post 62 IO mats that match 1947 industries
...
impute_import_mat <- function(use_df, use_row_trim_start, use_col_trim_end){
   ...
   import_pct_df <-
      ...
      mutate(
        ...
        domestic_supply = T007_Total_Commodity_Output + -1*(F050_Imports_of_goods_and_services) - F040_Exports_of_goods_and_services,
...
prepare_IO <- function(
    ...
    dir.create(total_commodity_output_dir)
    dir.create(consumption_output_dir)
    dir.create(total_industry_output_dir)
    dir.create(make_output_dir)
    dir.create(use_output_dir)
    dir.create(import_output_dir)
    ...
    for (y in seq(start_year, end_year)){
      ...
      make_df %>%
        slice(nrow(.)) %>%
        select(-c(industry, !!make_total_ind_output_colname)) %>%
      ...
      make_df %>%
        slice(-nrow(.)) %>%
        select(-!!make_total_ind_output_colname) %>%
      ...
      if (!is.na(import_file_path)){
        ...
        import_df <- impute_import_mat(use_df, use_row_trim_start, use_col_trim_end)
      }
...
prepare_IO(
    ...
    use_row_trim_start = 68,
    use_col_trim_end = 66,
    import_file_path = import97_xlsx_ipath
    )


# still the question of domestic consumption vector
...
### Attempting to determine if post1997 imports imputation works as I expect it

```{r eval=FALSE, include=FALSE}

use_df <- 
  read_xlsx(use97_xlsx_ipath, sheet = as.character(2000), skip = 5) %>%
  fix_names() %>%
  pivot_longer(-sector) %>%
  mutate(
    value = if_else(value == "...", "0", value),
...
import_df <- 
  read_xlsx(import97_xlsx_ipath, sheet = as.character(2000), skip = 5) %>%
  fix_names() %>%
  pivot_longer(-sector) %>%
  mutate(
    value = if_else(value == "...", "0", value),
...
import_mat <- 
  import_df %>%
  arrange(commodity) %>%
  select(all_of(colnames(.)[2:(which(colnames(.) == "T001_Total_Intermediate") - 1)])) %>%
  select(all_of(sort(colnames(.)))) %>%
  as.matrix()
...
use_mat <-
  ...
  slice(1:73) %>%
  arrange(commodity) %>%
  select(all_of(colnames(.)[2:(which(colnames(.) == "Total_Intermediate") - 1)])) %>%
  select(all_of(sort(colnames(.)))) %>%
  as.matrix()

div_mat <- import_mat / use_mat


use_c <- 
  use_df %>% 
  select(commodity, F010_Personal_consumption_expenditures) %>%
  rename(use_c = F010_Personal_consumption_expenditures)

import_c <- 
  import_df %>% 
  select(commodity,F010_Personal_consumption_expenditures) %>%
  rename(import_c = F010_Personal_consumption_expenditures)


join_df <- 
  left_join(use_c, import_c, by = "commodity") %>%
  mutate(frac_import = import_c/use_c)
...
import_pct_df <-
  ...
  mutate(
    ...
    import_pct_domestic_supply = -1*F050_Imports_of_goods_and_services/domestic_supply,
    import_pct_domestic_supply = as.character(100*if_else(positive_imports, 0, import_pct_domestic_supply))) %>%
  select(c(commodity, import_pct_domestic_supply, Total_Commodity_Output, F050_Imports_of_goods_and_services, domestic_supply, F040_Exports_of_goods_and_services))
