...
perform_ssp_analysis <- function(
  ...
  results_df <- 
    ...
    mutate(
      name = if_else(name == "Federal_Reserve_banks_credit_intermediation_and_related_activities", "Federal_Reserve_banks_credit_intermediation", name),
      year = analysis_year,
  ...
  inflation_impact_plot <- 
      ...
      scale_fill_manual(
        name = "",
        values = c("Indirect" = "darkblue", "Direct" = "gold"),
        breaks = c("Indirect", "Direct")
      ) + 
      scale_color_manual(
        guide = "none",
        values = c("Indirect" = "darkblue", "Direct" = "gold"),
...
ssp_comparison_wrapper <- 
  ...
  exogenous_industry_codes <- c("211", "212", "52", "531", "532RL", "55", "62", "722")
...
cropped_impact_plot <- function(results_df){
  
   inflation_impact_plot <- 
      results_df %>%
      filter(!is.na(impact_total)) %>%
      filter(impact_rank <= 15) %>%
      arrange(desc(impact_total)) %>%
      mutate(
        name = str_replace_all(name, "_", " "),
        num = str_pad(row_number(), 2, "left", "0"),
...
ssp_comparison <- function(latent_results, inflation_results, 
  ...
  dot_compare_plt <- 
    ...
    mutate(
      ...
      period = if_else(period == "pre", "pre-inflation", period),
      period = factor(period, levels = c("pre-inflation", "inflation"))
      ) %>%
    ggplot() + 
    ...
    theme(
      ...
      plot.subtitle = element_text(size = 8, color = "#566573")
    ) + 
    ...
    ylab("impact rank") + 
    labs(title = "Comparison of impact ranks", subtitle = "Black lines highlight top ten ranked\nindustries in either period") 
  ...
  cropped_latent_ssp_plot <- 
    cropped_impact_plot(latent_results[[1]]) + 
    labs(subtitle = glue("Latent SSP {latent_year_str}")) + 
    theme(plot.subtitle = element_text(size = 12))
  
  #ggsave(glue("{output_path}/cropped_latent_ssp.png"), cropped_latent_ssp_plot, width = 7, bg = "white")
  
  cropped_inflationary_period_ssp_plot <- 
    cropped_impact_plot(inflation_results[[1]]) + 
    labs(subtitle = glue("Inflationary Period SSP {inflation_year_str}")) + 
    theme(plot.subtitle = element_text(size = 12))
  
  #ggsave(glue("{output_path}/cropped_inflationary_ssp.png"), cropped_inflationary_period_ssp_plot, width = 7, bg = "white")
  
  cropped_dot_compare_plt <- 
    results_joint %>%
    filter(impact_rank.pre <= 15 | impact_rank.inflation <= 15) %>%
    mutate(
      highlight = ifelse(impact_rank.pre <= 10 | impact_rank.inflation <= 10, "T", "F"),
      alpha = ifelse(impact_rank.pre <= 10 | impact_rank.inflation <= 10, 1, 1)
      ) %>%
    ...
    mutate(
      ...
      period = if_else(period == "pre", "pre-inflation", period),
      period = factor(period, levels = c("pre-inflation", "inflation")),
      value = if_else(value >= 15, 15, value),
      value = fct_reorder(as.character(value), -value),
      value = fct_recode(value, "15+" = "15")
      ) %>%
    ggplot() + 
    geom_point(aes(x = period, y = value, color = highlight, fill = highlight)) +
    geom_line(aes(x = period, y = value, group = industry, color = highlight, size = highlight)) + 
  ...
  ggsave(glue("{output_path}/cropped_compare_ssp.png"), joint_plot, width = 14, bg = "white")
...
ranks_df <- 
  ssp_df %>%
  select(industry, name, impact_rank, 
         year, impact_total, share_personal_consump, 
         rank_fl, price_shock_pct) 
...
### Top SSP Component Plot

```{r}

stable_SSP_industry <- c("52", "311FT", "722", "62", "81", "531", "44RT")

top_SSP_component_df <- 
  ranks_df %>%
  filter(industry %in% stable_SSP_industry) %>%
  mutate(
    name = str_replace_all(name, "_", " "),
    industry = glue("[{industry}]"),
    code_name = glue("{name}  {industry}")
  ) %>%
  pivot_longer(c(share_personal_consump, price_shock_pct, rank_fl), names_to = "var") 
...
component_median_df <- 
  ...
  pivot_longer(c(share_personal_consump, price_shock_pct, rank_fl), names_to = "var") %>%
  group_by(year, var) %>%
  summarise(median_val = median(value, na.rm = T))

var_df <- 
  tibble(
    var = c("price_shock_pct", "share_personal_consump", "rank_fl"),
    title = c("Price Change (%)", "Share of Personal Consumption", "Forward Linkages (Rank)")
  )
...
for (i in 1:nrow(var_df)){
  ...
  plotlists[[i]] <- 
    ...
    theme_minimal() + 
    theme(
      strip.text.x = element_text(size = 8.5, color = "#333333"),
      #plot.title = element_text(size = 12)
      ) + 
    labs(title = ititle) + 
    xlab("")
  ...
  if (i != 3){
    plotlists[[i]] <-
      ...
      ylab("%") +
      geom_line(
        data = component_median_df %>% filter(var == ivar),
        aes(x = year, y = median_val),
        color = "red"
      ) + 
      labs(title = glue("{ititle} - red line indicates median value for other sectors"))
  }
  
}

top_SSP_component_plot <- plot_grid(plotlist = plotlists, ncol = 1)
ggsave(glue("{proj_root}/results/long_term/top_SSP_components.png"), top_SSP_component_plot, bg = "white", width = 18, dpi = 600)
  
```


### Temporary SSP Component Plot

```{r}

temp_SSP_industry <- c("51", "54", "42", "22")

temp_SSP_component_df <- 
  ranks_df %>%
  filter(industry %in% temp_SSP_industry) %>%
  mutate(
    name = str_replace_all(name, "_", " "),
    industry = glue("[{industry}]"),
    code_name = glue("{name}  {industry}")
...
for (i in 1:nrow(var_df)){
  ...
  plotlists[[i]] <- 
    ...
    facet_grid(cols = vars(name)) + 
    theme_minimal() + 
    theme(strip.text.x = element_text(size = 8, color = "#333333")) + 
    labs(title = ititle) + 
    xlab("")
  
  if (i == 3){
     plotlists[[i]] <-
      plotlists[[i]] + scale_y_reverse(limits = c(47, 1)) +
       ylab("rank") 
  }
...
temp_SSP_component_plot <- plot_grid(plotlist = plotlists, ncol = 1)
ggsave(glue("{proj_root}/results/long_term/temp_SSP_components.png"), temp_SSP_component_plot, bg = "white", width = 18)
...
file.copy(
  ...
  glue("{proj_root}/results/publish/fig02-impact_line_plot.png"),
  overwrite = T
)

file.copy(
  glue("{proj_root}/results/long_term/impact_dot_plot.png"),
  glue("{proj_root}/results/publish/fig03-impact_dot_plot.png"),
  overwrite = T
)

file.copy(
  glue("{proj_root}/results/long_term/top_SSP_components.png"),
  glue("{proj_root}/results/publish/fig04-top_SSP_components.png"),
  overwrite = T
)

file.copy(
  glue("{proj_root}/results/long_term/temp_SSP_components.png"),
  glue("{proj_root}/results/publish/fig05-temp_SSP_components.png.png"),
  overwrite = T
)
...
gt_df <- 
  tibble(
    period = c(rep("1950-51", 6), rep("1956-58", 4), rep("1967-70", 7), rep("1973-75", 8),  rep("1978-80", 4)),
    sector = c("Food and Beverage and Tobacco Products", "Farms", "Retail Trade", "Real Estate", "Apparel and Leather and Allied Products", "Wholesale Trade", "Food and Beverage and Tobacco Products", "Retail Trade", "Real Estate", "Farms", "Retail Trade", "Real Estate", "Food and Beverage and Tobacco Products", "Food Services and Drinking Places", "Other Services Except Government", "Federal Reserve Banks Credit Intermediation", "Hospitality and Nursing and Residential Care Facilities", " Ambulatory Health Services", "Retail Trade", "Petroleum and Coal Products", "Food and Beverage and Tobacco Products", "Real Estate", "Oil and Gas Extraction", "Chemical Products", "Utilities", "Petroleum and Coal Products", "Retail Trade", "Oil and Gas Extraction", "Real Estate")
  )

sum_df <- 
  gt_df %>%
  group_by(sector) %>%
  summarise(n_periods_ssp = n()) 

gt_df %>%
  mutate(check = "✔") %>%
  pivot_wider(sector, names_from = period, values_from = c(check)) %>%
  left_join(sum_df, by = "sector") %>%
  arrange(desc(n_periods_ssp)) %>%
  select(-n_periods_ssp) %>%
  ...
  cols_width(
    starts_with("19") ~ px(100),
  ) %>%
  tab_header(
    title = "SSPs Across Inflationary Periods",
  ) %>%
  cols_align(align = "center", columns = -Sector) %>%
  gtsave(glue("{proj_root}/results/period/tbl.png"), expand = 10)

```

```{r}

exogenous_industry_codes47 <- c("211", "212", "52", "531", "532RL", "55", "62", "722")

exogenous_industry_codes63 <- 
  c("211", "212", "521CI", "523", "524", "525", 
    "531", "532RL","55", "621", "622HO", "713", "722")

industry_codes63_df <- 
  read_csv(glue("{proj_root}/data/prepared/industry_codes_1963_matched.csv")) 
